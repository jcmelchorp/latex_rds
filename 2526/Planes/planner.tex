...
\begin{minipage}[t][12cm][t]{0.76\textwidth}
    \tcbsubtitle{INICIO}
    \tcboxfit[colback=white,colframe=white,top=0mm,bottom=0mm,left=0mm,right=10pt,height=2.8cm]{#1}
    \tcbsubtitle{DESARROLLO}
    \tcboxfit[colback=white,colframe=white,top=0mm,bottom=0mm,left=0mm,right=10pt,height=4.5cm]{#2}
    \tcbsubtitle{CIERRE}
    \tcboxfit[colback=white,colframe=white,top=0mm,bottom=0mm,left=0mm,right=10pt,height=1cm]{#3}
\end{minipage}\hfill
\begin{minipage}[t][12cm][t]{0.24\textwidth}
    \begin{tcbraster}[raster columns=3]
        \begin{titlebox}{Actividades}\centering
            #4
        \end{titlebox}
        \begin{titlebox}[minipage boxed title*=-0.5cm,attach boxed title to top center = {yshift=-\tcboxedtitleheight/4},colback=white]{\scriptsize Cuaderno de Evidencias}\centering
            #5
        \end{titlebox}
        \begin{titlebox}{Proyectos}\centering
            #6
        \end{titlebox}
    \end{tcbraster}
...
\begin{tcolorbox}[titlebox,title=\RaggedLeft\planTitle
        ...
        \weekcontents
            ...
            {\tcbox[on line,top=2mm,right=2mm,bottom=2mm,left=2mm,arc=8pt]{\JSONParseValue{\weekdata}{weeks[#1].Evidencias}}}
            {\tcbox[size=fbox,on line]{\JSONParseValue{\weekdata}{weeks[#1].Proyecto}}}
            {%
            \begin{itemize}[itemsep=0pt,itemindent=-2em]
                \item Física, Imagina, págs. \JSONParseValue{\weekdata}{weeks[#1].LibroMcMillan}
                \JSONParseArrayMapInline{\weekdata}{weeks[#1].LibroSep}{%
                   \item \JSONParseValue{\weekdata}{weeks[#1].LibroSep[##1]}
                }%
            \end{itemize}%
            }
            {\JSONParseValue{\weekdata}{weeks[#1].Vinculacion}}
\end{tcolorbox}
\weekbottom
